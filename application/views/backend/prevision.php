...
function grid<?=$nombreTab?>Reload(){//@
	...
	//@
	jQuery("#grid<?=$nombreTab?>").setGridParam({url:"index.php?/<?=$controller?>/grid/<?=$keyTab?>",postData:{"livesearch":true,"f_id":f_id,"f_nombre":f_nombre,"f_tipo":f_tipo},page:1}).trigger("reloadGrid");
}
//////////////////////
<?php } ?>
...
function form<?=$nombreTab?>Edit(id){
	...
	$('#ib_prevision_id').show();
	$('#clean_prevision_id').show();
}
...
function fk<?=$nombreTab?>Default(){//NO TOCAR
	$('#form<?=$nombreTab?>Standard')[0].prevision_id.value=<?=$keyTab?>;//@
	$('#form<?=$nombreTab?>Standard')[0].prevision_id_aux.value=<?=$keyTab?>;//@
	$('#ib_prevision_id').hide();
	$('#clean_prevision_id').hide();
}
///////
<?php } ?>
