...
<div id="dataForm<?=$nombreTab?>" style="display:none">
	...
	<table  class="tabla_form" >
		...
		<tr>
			<th>FECHA</th>
			<td><input type="text" id="fecha" name="fecha" value="" class="required"  ></input></td>
		</tr>
		<tr>
			<th>OBSERVACION</th>
			<td><input type="text" size="30" id="observacion" name="observacion" value="" class="required"  ></input></td>
		</tr>
		<tr>
...
function grid<?=$nombreTab?>Reload(){//@
	...
	//@
	jQuery("#grid<?=$nombreTab?>").setGridParam({url:"index.php?/<?=$controller?>/grid/<?=$keyTab?>",postData:{"livesearch":true,"f_id":f_id,"f_fecha":f_fecha,"f_observacion":f_observacion},page:1}).trigger("reloadGrid");
}
//////////////////////
<?php } ?>
...
function form<?=$nombreTab?>Edit(id){
	...
	$('#ib_item_ficha_clinica_id').show();
	$('#clean_item_ficha_clinica_id').show();
}
...
<?php if(!$inputBuscador){?>
//CALL INPUT BUSCADOR 
$('#ib_item_ficha_clinica_id').click(function()
	{
		$.ajax({
			type: "POST",
			url: 'index.php?/item_ficha_clinica_controller/cargaModulo/IBItem_ficha_clinica/item_ficha_clinica_id',
			success: function(datos){
...
function fk_lugar_id(valor){//@
	$('#form<?=$nombreTab?>Standard')[0].item_ficha_clinica_id.value=valor;//@
	$('#form<?=$nombreTab?>Standard')[0].item_ficha_clinica_id_aux.value=valor+fkName('IBItem_ficha_clinica',valor,2);//@ 
	$("#ib").hide();
}
