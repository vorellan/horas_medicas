...
class Prevision_model extends CI_Model {//@
	...
	function grid($p,$o){
		...
		//PAGINADOR
		$queryCount="SELECT COUNT(*) AS count FROM prevision ".$where;//@
		...
		//@
		$queryData = "SELECT * FROM prevision ".$where." ORDER BY $p->sidx $p->sord LIMIT $p->limit offset $start "; 
        ...
        if ($result->num_rows() > 0)
		{
			...
			foreach($result->result() as $rs){//@
				...
				$response->rows[$i]['cell']=array($rs->id,$rs->nombre,$rs->tipo);  
				$i++;
			}
	...
	function insertar($o){
		...
		$result=$this->db->insert("prevision",$query);//@
	...
	function editar($key,$o){
		...
		$result=$this->db->update("prevision",$query);//@
	...
	function verEntidad($key){
        ...
        $query = "SELECT * FROM prevision as p WHERE p.id='$key' ";
	...
	function eliminar($key){
		...
		$this->db->where("id",$key);//@
		$result=$this->db->delete("prevision");//@
